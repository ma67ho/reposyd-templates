...
/* reposyd-reportblock-header */
class ReportBlockWBSDictionary extends ReportBlock {
  ...
  renderContent() {
    ...
    if ( wbs.length === 0 ) {
      ...
      const sectionNumber = new LexicographicalOrder.SectionNumber( this.section.number )
      sectionNumber.add()
      ...
      function renderDeliverables( wp ) {
        ...
        if ( links.length === 0 ) {
          ...
          links
            .sort( ( a, b ) => {
              return LexicographicalOrder.compare( a.ddo.attributes.number, b.ddo.attributes.number )
            } )
            .forEach( link => {
